...
function createSamplePdfBuffer(fileId: string): Buffer {
...
const handleSetupIntent = async (req: Request, res: Response): Promise<void> => {
  try {
...
const handleStatementsList = async (req: Request, res: Response): Promise<void> => {
  try {
    if (process.env.STRIPE_SECRET_KEY && process.env.STRIPE_SECRET_KEY.startsWith('sk_')) {
      try {
        ...
        const statements = await Promise.all(
          reportRuns.data.map(async (run) => {
            let filename = `stripe_statement_${run.id}.pdf`;
            const fileId = typeof run.result === 'string' ? run.result : (run.result as any)?.id || run.id;
            let fileSize = '1.4 MB';

            if (run.result) {
              try {
                const resultId = typeof run.result === 'string' ? run.result : (run.result as any)?.id;
                if (resultId) {
...
const handleStatementView = async (req: Request, res: Response): Promise<void> => {
  try {
    const fileId = req.params.fileId;

    if (process.env.STRIPE_SECRET_KEY && process.env.STRIPE_SECRET_KEY.startsWith('sk_') && fileId) {
      try {
        const fileStream = await (stripe.files as any).retrieveContents(fileId);
        const buffer = Buffer.isBuffer(fileStream)
          ? fileStream
          : Buffer.from(typeof fileStream === 'string' ? fileStream : (fileStream as any));
